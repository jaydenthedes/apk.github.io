...
function gcmServicesEnabled() {
  var services = arcMetadata.getValue('usePlayServices');
...
function GcmManager() {
  /**
   * True if the plugin is ready to receive an androidGCM message.
   * @private
   */
  this.gcmInitialized_ = false;
}
...
GcmManager.prototype.onMessageInternal_ = function(message_type, opt_data) {
  ...
  try {
    ...
    if (!plugin) {
      ...
      if (this.gcmInitialized_) {
        plugin.postMessage(json);
      } else {
        // TODO(victorhsieh): It would be nice to queue up messages and flush on
        // the init notification.
        console.warn('GCM has not initialized yet');
      }
    }
  } catch (e) {
