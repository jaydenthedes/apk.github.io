...
var _HEARTBEAT_STACK_TRACES_TIMEOUT = 3000;  // 3s
...
/** @const
 *
 * The argument to launch FileHandlerLaunchHelperActivity.
 */
var _FILE_HANDLER_LAUNCH_ARG = [
  'am', 'start', '-n',
  'org.chromium.arc.helper/.FileHandlerLaunchHelperActivity', ';'];
...
Plugin.prototype.init = function() {
  ...
  if (isAppCwsInstalled()) {
    chrome.runtime.getPlatformInfo(function(info) {
      if (info.os != 'cros' || info.nacl_arch == 'x86-32')
 ...
 * @public
 *
 * Adds a callback which will be called when the app plugin is ready to use.
 *
 * @param callback Callback to be called when the app is ready.
 */
Plugin.prototype.addAppPluginReadyCallback = function(callback) {
  if (this.appPluginReady_) {
    console.error('Plugin is already ready to use');
    return;
  }
  this.onAppPluginReadyCallbacks_.push(callback);
};
 ...
 * @return A Promise to be resolved when the shell command is executed. The
 *         resolve object is a session object representing the shell session.
 *
 * Calling close() on the resolved object can be used to close the session.
 *
 * Example:
 *    // Request a shell session to run logcat. This may execute later when the
 *    // runtime is not ready to execute the shell.
 *    promise = plugin.shell('logcat');
 *    promise.then(function(session) {
 *      session.close(); // close the shell session.
 *    } );
 *
 *    or:
 *    promise = plugin.shell('ls', onDataCallback, onClosedCallback);
 *
 */
  ...
  if (this.metadata_.packageName == 'org.chromium.arc.system') {
    ...
    if (this.metadata_.shell.length > 0) {
      ...
      if (window['arc'].launchArgs.items) {
        ...
        this.shellCommands_ =
            this.shellCommands_.concat(_FILE_HANDLER_LAUNCH_ARG);
      } else {
...
Plugin.prototype.handleAppPluginLoadAttempt_ = function(pluginObject) {
  ...
  pluginObject.setAttribute('enableaccessibility',
                            this.metadata_.enableAccessibility);
  pluginObject.setAttribute('enableadb', this.metadata_.enableAdb.toString());
  pluginObject.setAttribute('enablecompositor',
                            this.metadata_.enableCompositor.toString());
  ...
  pluginObject.setAttribute('disableglfixedattribs',
                            this.metadata_.disableGlFixedAttribs.toString());
  ...
  pluginObject.setAttribute('canrotate', this.metadata_.canRotate);
  ...
  pluginObject.setAttribute('isappcwsinstalled', isAppCwsInstalled());
  pluginObject.setAttribute('javatracestartup',
                            this.metadata_.javaTraceStartup);

  // We are about to enable Native Client to start loading, so we end the
  // JS init phase and start the Plugin Load phase.
  ...
  console.log('Compositor is ' +
      (this.metadata_.enableCompositor ? 'enabled' : 'disabled'));
...
Plugin.prototype.handleAppPluginReady_ = function() {
  ...
  // Registers JavaScript namespace to plugin to obtain messages.
  var namespaces = Object.keys(this.messageListeners_);
  this.postMessage({
    namespace: 'pluginJSMessageProxy',
    command: 'registerJsNamespaces',
  ...
  for (var i = 0; i < this.onAppPluginReadyCallbacks_.length; ++i) {
    this.onAppPluginReadyCallbacks_[i]();
  }
};
...
/** @private */
Plugin.prototype.reportCrash_ = function(message, minidump, opt_extraData) {
  this.reportAppShutdown_(true);

  var extraCrashData = opt_extraData || {};
  /** @type {AppWindow} */
  var currentAppWindow = chrome.app.window.current();
  if (currentAppWindow) {
    extraCrashData.width = currentAppWindow.getBounds().width;
    extraCrashData.height = currentAppWindow.getBounds().height;
    extraCrashData.is_minimized = currentAppWindow.isMinimized();
    extraCrashData.is_maximized = currentAppWindow.isMaximized();
    extraCrashData.is_fullscreen = currentAppWindow.isFullscreen();
  }
  ...
  pluginObject.addEventListener('message', (function(messageEvent) {
    ...
    if (message.namespace == 'jsPlugin') {
      if (message.command == 'loadResult') {
        console.timeEnd('ARC Plugin Load');
        if (message.data.result) {
          ...
          this.crashExtraInformation_['sig'] = 'LoadFailure';
        ...
        if (this.crashExtraInformation_['sig'] === undefined) {
          // Android should set its own signature, but provide a fallback in
          // case something went wrong.
          this.crashExtraInformation_['sig'] = 'AndroidUnhandledException';
        }
...
Plugin.prototype.heartbeat_ = function() {
  ...
  if (this.waitingForHeartbeat_) {
    ...
    // If the heartbeat monitor has not gotten a response back from the plugin
    // for two heartbeats in a row, we treat it as a crash and request the Java
    // stack traces for all threads.  This is done with a small timeout in case
    // the system is in a very unstable state and cannot respond even to this
    // message.
    this.heartbeatStackTraceTimeout_ =
        setTimeout(this.heartbeatSendCrash_.bind(this),
                   _HEARTBEAT_STACK_TRACES_TIMEOUT);
    this.postMessage(
        {namespace: 'pluginUtil', command: 'heartbeatStackTrace', 'data': {}});
  } else {
 ...
 * @private
 *
 * Stop waiting for the stack traces (in case they have not arrived yet) and
 * report the heartbeat failure to the crash server.
 */
Plugin.prototype.heartbeatSendCrash_ = function() {
  if (!this.heartbeatStackTraceTimeout_)
    return;
  clearTimeout(this.heartbeatStackTraceTimeout_);
  this.heartbeatStackTraceTimeout_ = null;
  this.showCrash_('heartbeat ping timeout');
  this.crashExtraInformation_['sig'] = 'HeartbeatTimeout';
...
Plugin.prototype.handleDisplayMessage_ = function(message) {
  if (message.command == 'config') {
    ...
    this.showNotSupported_();
    this.crashExtraInformation_['sig'] = 'MissingGPUSupport';
    this.reportCrash_('Missing GPU support', null);
  } else if (message.command == 'showInvalidPlayServices') {
    this.showNotSupported_();
    var error = message.data ? message.data.errorCode : -1;
    var data = {'play_services_error_code': error};
    this.crashExtraInformation_['sig'] = 'InvalidPlayServices';
    this.reportCrash_('Invalid Play Services', null, data);
  } else {
...
Plugin.prototype.handleMediaStreamMessage_ = function(message) {
  if (message.command == 'audioOpenCall') {
    if (this.audioTrack_ != null) {
      ...
      var responseMessage = {
        namespace: message.data.requester,
        command: 'rpcReturn',
        data: {
...
Plugin.prototype.handleHeartbeatMessage_ = function(message) {
  if (message.command == 'pong') {
    ...
    this.crashExtraInformation_['upload_file_stack_traces'] =
        message.data.upload_file_stack_traces;
    // Once the traces have arrived, send the crash to the server.
    this.heartbeatSendCrash_();
  } else {
...
Plugin.prototype.initializeTopBar_ = function(topbar) {
  ...
  // Set the correct direction for the buttons.
...
Plugin.prototype.onRelaunched = function(args) {
  ...
  if (args.items) {
    ...
    this.shell(_FILE_HANDLER_LAUNCH_ARG.join(' '));
  }
 ...
 * @private
 *
 * Handles in-app purchase messages.
 *
 * @param message IAP message to handle.
 */
Plugin.prototype.handleIapMessage_ = function(message) {
  var generalCallback = (function(response, command) {
    this.postMessage({
      namespace: 'javaIap',
      command: command,
      data: response
  ...
  if (message.command == 'getSkuDetails') {
    var callback = function(response) {
      generalCallback(response, 'getSkuDetailsResponse');
    };
    ...
    var callback = function(response) {
      generalCallback(response, 'buyResponse');
    };
  ...
  } else if (message.command == 'getPurchases') {
    var callback = function(response) {
      generalCallback(response, 'getPurchasesResponse');
    };
  ...
  } else if (message.command == 'consumePurchase') {
    var callback = function(response) {
      generalCallback(response, 'consumePurchaseResponse');
    };
    ...
    console.error('Received unknown IAP command: ' + message.command);
  }
};
...
if (window['arc'].systemDirectoriesReady) {
  window['arc'].systemDirectoriesReady =
      PromiseBridge.createPromise(window['arc'].systemDirectoriesReady);
}
