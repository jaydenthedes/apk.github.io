...
// Promises to be fullfilled with an array of system directory metadata or an
// empty array.
var systemDirectoriesCreated = null;
...
function installLogic() {
  ...
  var data_directory = applyPromise(
      [root, 'data', {create: true}],
      PromiseWrap.getDirectory);
  var dalvik_directory = applyPromise(
      [data_directory, 'dalvik-cache', {create: true}],
      PromiseWrap.getDirectory);

  dalvik_directory.then(function() {
...
// to a new version, when Chrome is updated to a new version, and the runtime is
// updated.
