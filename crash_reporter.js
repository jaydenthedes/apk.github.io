...
CrashReporter.prototype.sendCrashReport_ = function(crash_data) {
  ...
  for (var item in crash_data) {
    if (item.indexOf('upload_file_') === 0) {
      // Treat all fields that start with 'upload_file_' as attachments.
...
/**
 * Print minidump data to console, for development.
 * @param minidump Crash dump data
 * @param {!string} version string to be used as minidump file name.
 * @private
 */
function printMiniDump_(minidump, version) {
  if (minidump != null) {
    var data = '';
    ...
    // The size of base64Data is around 10K bytes, which is not too
    // huge to be output to the log.  This log is scraped by
    // launch_chrome in minidump_filter.py, so the format should
    // match.
    console.error('@@@Minidump generated@@@' +
        'minidump_' + version + '@@@' +
        base64Data + '@@@');
  }
}
...
CrashReporter.prototype.reportCrash = function(
  ...
  return requestRuntimeManifest().then(function(manifest) {
  ...
  }).then(function(should_send_crash) {
    ...
    // Do not send crash reports from development builds.  Instead, log those
    // crash reports locally.
    if (should_send_crash) {
